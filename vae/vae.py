...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        self.encoder = nn.Sequential(
          nn.Flatten(),
          nn.Linear(self.input_size+10, self.hidden_dim),
          nn.ReLU(),
          nn.Linear(self.hidden_dim, self.hidden_dim),
          nn.ReLU(),
          nn.Linear(self.hidden_dim, self.hidden_dim),
        ...
        # Replace "pass" statement with your code
        self.decoder = nn.Sequential(
          nn.Linear(self.latent_size+10, self.hidden_dim),
          nn.ReLU(),
          nn.Linear(self.hidden_dim, self.hidden_dim),
          nn.ReLU(),
          nn.Linear(self.hidden_dim, self.hidden_dim),
    ...
    def forward(self, x, c):
        ...
        encoder_catted = torch.cat((torch.reshape(x,(x.shape[0], x.shape[2]*x.shape[3])),c),1)

        h = self.encoder(encoder_catted)
        mu = self.mu_layer(h) 
        logvar = self.logvar_layer(h) 
        z = reparametrize(mu, logvar)
        decoder_catted = torch.cat((z,c),1)
        x_hat = self.decoder(decoder_catted)
        ############################################################################################
        #                                      END OF YOUR CODE                                    #
        ############################################################################################
        return x_hat, mu, logvar
...
def reparametrize(mu, logvar):
    ...
    epsilon = torch.randn_like(std) # sampling again for each matrix element (not using the same sampled value across all elements)
    z = mu + std*epsilon
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    reconst_loss = nn.functional.binary_cross_entropy(x_hat, x, reduction='sum')/x.shape[0] # input, target. normalize by the batch size but only the dim we care about
    kl_vec = 1 + logvar - torch.square(mu) - torch.exp(logvar)
    kl_loss = -0.5*torch.sum(kl_vec)/x.shape[0] # normalize by the batch size
