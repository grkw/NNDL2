...
class DQN:
    ...
    def _optimize_model(self):
        ...
        if len(self.replay_buffer) < 10 * self.batch_size:
            return False, 0.0  # Not enough samples to optimize the model
            ...
            not_dones = ~ dones #determines if terminal? always true at the start, which is what is observed

            # print(not_dones)

            # y_j = rewards.clone()

            # next_state_q_vals = self.model(next_states)
            
            
            # future_max_q = next_state_q_vals.max(dim=1)[0].detach()

            # future_max_q = self.model(next_states).max(1)[0]

            # y_j[not_dones] += (self.gamma * future_max_q[not_dones])


            with torch.no_grad():
                next_q_values = self.model(next_states)
                next_q_values = next_q_values.max(1)[0]
                y_j = rewards + self.gamma * next_q_values * (~dones)

            # y_j = rewards + (not_dones) * self.gamma * future_max_q


            all_q_values = self.model(states)
            # print(actions)
            actions = actions.unsqueeze(1)
            # q_values = all_q_values[range(self.batch_size), actions]
            q_values = all_q_values.gather(1, actions).squeeze(1)



                    # q_values = torch.gather(self.model(current_states), 1, actions.unsqueeze(1)).squeeze(-1)

            loss = self.loss_fn(q_values, y_j)

            # loss = ((y_j - q_values) ** 2).mean()
