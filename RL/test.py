...
def test_model_DQN(model):
    try:
        ...
        model.load_state_dict(torch.load("test_weights.pt",map_location=torch.device('cpu')))
        # Test the forward function
        test_outputs =  torch.load("test_outputs.pt",map_location=torch.device('cpu'))
...
def test_wrapper(wrapper):
    try:
        ...
        assert np.allclose(test_outputs[0],s), f"expected {test_outputs} but got {s}"
...
def test_DQN_replay_buffer(buffer_class):
    with open("test_replay_buffer_inputs.pkl","rb") as f:
        buffer_inputs = pickle.load(f)
    buffer_samples = torch.load("test_replay_buffer_samples.pth")
    try:
